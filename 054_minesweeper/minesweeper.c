...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  // 1. create board_t and its "int ** board", and initialize them
  board_t * a = malloc(sizeof(*a));
  a->width = w;
  a->height = h;
  a->totalMines = numMines;
  a->board = malloc(h * sizeof(*(a->board)));
  for (int i = 0; i < h; i++) {  // in each row
    a->board[i] = malloc(w * sizeof(**(a->board)));
    // fill in "int ** board"
    for (int j = 0; j < w; j++) {  // in each element/column
      a->board[i][j] = UNKNOWN;
    }
  }
...
int countMines(board_t * b, int x, int y) {
  ...
  int ny, nx;
  int w = b->width;   //for x
  int h = b->height;  //for y
  int count = 0;
  for (int off_y = -1; off_y <= 1; off_y++) {
    for (int off_x = -1; off_x <= 1; off_x++) {
      ny = y + off_y;
      nx = x + off_x;
      // continue the loop if goes out of bounds of the array
      if (off_x == 0 && off_y == 0) {
        continue;
      }
      else if (ny < 0 || nx < 0 || ny == h || nx == w) {
        continue;
      }
      else {  // in boundary
        if (IS_MINE(b->board[ny][nx]) == 1) {
          count++;
        }
      }
    }
...
int checkWin(board_t * b) {
  ...
  int h = b->height;
  for (int i = 0; i < h; i++) {    // in each row
    for (int j = 0; j < w; j++) {  // each element/col
